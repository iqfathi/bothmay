...
setInterval(() => {
  http.get(`http://iqfahti.glitch.me/`);
}, 280000);
...
const youtube = new YouTube("AIzaSyAqPdd8p3AiIKx1h9gWH908argJWIyh5kY"); //تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@616677409638449168>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
\`شرح البوت \` : <https://www.youtube.com/c/IQFATHI/videos>
`);
        })
        .then(e => {
          message.react("✅");
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var channel = member.guild.channels.find(gg => gg.name === "pro-logs⛔"); //// تعديل اساسي
      if (!channel) return;
